...
# Cette fonction résout numériquement l'équation de Schrödinger stationnaire
# pour un potentiel donné en utilisant la méthode de discrétisation par éléments finis.
def solve_stationary_states(V, dx, Nstates=5):
    diag = 1.0 / dx**2 + V  # diagonale principale du Hamiltonien
    off_diag = -0.5 / dx**2 * np.ones(N - 1)  # diagonales secondaires
    energies, wavefuncs = eigh_tridiagonal(diag, off_diag)  # solveur des états propres
    return energies[:Nstates], wavefuncs[:, :Nstates]

# Calcul des premiers états stationnaires
...
# Affichage des énergies trouvées
print("
États stationnaires trouvés :")
for i, E in enumerate(energies):
    print(f"État {i} : Énergie = {E:.4f}")

# Affichage graphique des fonctions d'onde associées
